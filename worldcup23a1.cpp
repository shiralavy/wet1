                                   ...
                                   int goals, int cards, bool goalKeeper)
{
    ...
    { // the player does not exist, the team does exist
        try
        {
            ...
            insertedPlayerByScore->m_element.m_player = insertedPlayer;
...
StatusType world_cup_t::remove_player_except_from_tree_ready_teams(int playerId)
{
    ...
    int nextPlayerID = 0;
    int nextPlayerGoals = 0;
    int nextPlayerCards = 0;
    int prevPlayerID = 0;
    int prevPlayerGoals = 0;
    int prevPlayerCards = 0;
    if (playerToRemoveByScore->m_element.m_next_player_by_score){
        nextPlayerID = playerToRemoveByScore->m_element.m_next_player_by_score->m_key3;
        nextPlayerGoals = playerToRemoveByScore->m_element.m_next_player_by_score->m_key1;
        nextPlayerCards = playerToRemoveByScore->m_element.m_next_player_by_score->m_key2;
    }
    if (playerToRemoveByScore->m_element.m_prev_player_by_score){
        prevPlayerID = playerToRemoveByScore->m_element.m_prev_player_by_score->m_key3;
        prevPlayerGoals = playerToRemoveByScore->m_element.m_prev_player_by_score->m_key1;
        prevPlayerCards = playerToRemoveByScore->m_element.m_prev_player_by_score->m_key2;
    }
    playerToRemoveByScore->m_element.m_next_player_by_score = nullptr;
    playerToRemoveByScore->m_element.m_prev_player_by_score = nullptr;
    ...
    Node<player_in_scoreboard>* prevPlayer = this->m_tree_players_by_score->findNode(this->m_tree_players_by_score->m_root, prevPlayerGoals, prevPlayerCards,prevPlayerID);
    if (prevPlayer){
        prevPlayer->m_element.m_next_player_by_score = nullptr;
        prevPlayer->m_element.m_next_player_by_score = this->m_tree_players_by_score->closestHigherNode(this->m_tree_players_by_score->m_root, prevPlayer);
        prevPlayer->m_element.m_prev_player_by_score = nullptr;
        prevPlayer->m_element.m_prev_player_by_score = this->m_tree_players_by_score->closestLowerNode(this->m_tree_players_by_score->m_root, prevPlayer);
        prevPlayer->m_element.m_player->m_element.m_player_in_scoreboard = prevPlayer;
    }
    Node<player_in_scoreboard>* nextPlayer = this->m_tree_players_by_score->findNode(this->m_tree_players_by_score->m_root, nextPlayerGoals, nextPlayerCards,nextPlayerID);
    if (nextPlayer){
        nextPlayer->m_element.m_prev_player_by_score = nullptr;
        nextPlayer->m_element.m_prev_player_by_score = this->m_tree_players_by_score->closestLowerNode(this->m_tree_players_by_score->m_root, nextPlayer);
        nextPlayer->m_element.m_next_player_by_score = nullptr;
        nextPlayer->m_element.m_next_player_by_score = this->m_tree_players_by_score->closestHigherNode(this->m_tree_players_by_score->m_root, nextPlayer);
        nextPlayer->m_element.m_player->m_element.m_player_in_scoreboard = nextPlayer;

    }

    //Node<player_in_team> *tempPlayerInTeamByScore = playerToRemove->m_element.m_player_in_team_by_score;
    Node<Team> *tempTeamContainingPlayer = playerToRemove->m_element.m_my_team;
...
// delete from tree of players by score inside Team
    Node<player_in_team>* nextInTeamByScore = tempTeamContainingPlayer->m_element.m_tree_players_in_team_by_score->closestHigherNode(tempTeamContainingPlayer->m_element.m_tree_players_in_team_by_score->m_root, playerToRemove->m_element.m_player_in_team_by_score);
    int nextByScoreGoals = 0; int nextByScoreCards = 0;
    int nextByScoreId = 0;
    if (nextInTeamByScore){
        nextByScoreGoals = nextInTeamByScore->m_key1;
        nextByScoreCards = nextInTeamByScore->m_key2;
        nextByScoreId = nextInTeamByScore->m_key3;
    }
    ...
    nextInTeamByScore = tempTeamContainingPlayer->m_element.m_tree_players_in_team_by_score->findNode(tempTeamContainingPlayer->m_element.m_tree_players_in_team_by_score->m_root, nextByScoreGoals, nextByScoreCards, nextByScoreId);
    if (nextInTeamByScore){
        nextInTeamByScore->m_element.m_player->m_element.m_player_in_team_by_score = nextInTeamByScore;
    }

    // delete from tree of players by id inside team
    Node<player_in_team>* nextInTeamById = tempTeamContainingPlayer->m_element.m_tree_players_in_team_by_id->closestHigherNode(tempTeamContainingPlayer->m_element.m_tree_players_in_team_by_id->m_root, playerToRemove->m_element.m_player_in_team_by_id);
    int nextInTeamID = 0;
    if (nextInTeamById){
        nextInTeamID = nextInTeamById->m_key1;
    }
    ...
    nextInTeamById = tempTeamContainingPlayer->m_element.m_tree_players_in_team_by_id->findNode(tempTeamContainingPlayer->m_element.m_tree_players_in_team_by_id->m_root, nextInTeamID,0,0);
    if (nextInTeamById){
        nextInTeamById->m_element.m_player->m_element.m_player_in_team_by_id = nextInTeamById;
    }
    // delete from tree of all players by playerId
    Node<player>* nextById = this->m_tree_players_by_id->closestHigherNode(this->m_tree_players_by_id->m_root, playerToRemove);
    int nextID = 0;
    if (nextById){
        nextID = nextById->m_key1;
    }
    this->m_tree_players_by_id->m_root = this->m_tree_players_by_id->deleteNode(this->m_tree_players_by_id->m_root, playerId, 0, 0);
    nextById =  this->m_tree_players_by_id->findNode(this->m_tree_players_by_id->m_root,nextID,0,0 );
    if (nextById){
        nextById->m_element.m_player_in_scoreboard->m_element.m_player = nextById;
        nextById->m_element.m_player_in_team_by_score->m_element.m_player = nextById;
        nextById->m_element.m_player_in_team_by_id->m_element.m_player = nextById;
    }
    this->m_best_player = this->m_tree_players_by_score->m_highest_key3;
    this->m_num_players--;
    return StatusType::SUCCESS;
...
StatusType world_cup_t::add_team(int teamId, int points)
{
    ...
    else
    {
        try
        {
            ...
            newTeam.m_tree_players_in_team_by_score = nullptr;
            newTeam.m_tree_players_in_team_by_id = nullptr;
        }
        catch (std::bad_alloc &)
        {
            return StatusType::ALLOCATION_ERROR;
...
StatusType world_cup_t::remove_team(int teamId)
{
    ...
    // this checks if the team does not exist or if the team is empty
...
StatusType world_cup_t::remove_player(int playerId)
{
    ...
    if (playerToRemove->m_element.m_goalkeeper)
    {
                                            ...
                                            int scoredGoals, int cardsReceived)
{
    ...
    Node<Team> *prevTeam = playerToUpdate->m_element.m_my_team;
...
output_t<int> world_cup_t::get_num_played_games(int playerId)
{
    ...
    int numGamesPlayedWithTeam = playerToBringGames->m_element.m_my_team->m_element.m_games_played_by_team;
...
StatusType world_cup_t::unite_teams(int teamId1, int teamId2, int newTeamId) {
    ...
    team1->m_element.m_tree_players_in_team_by_id->inOrderVisitUnite(
            team1->m_element.m_tree_players_in_team_by_id->m_root, arrayPlayersID1, 0);
    team1->m_element.m_tree_players_in_team_by_score->inOrderVisitUnite(
            team1->m_element.m_tree_players_in_team_by_score->m_root, arrayPlayersScore1, 0);
    team2->m_element.m_tree_players_in_team_by_id->inOrderVisitUnite(
            team2->m_element.m_tree_players_in_team_by_id->m_root, arrayPlayersID2, 0);
    ...
    if (newTeamId != teamId1 && newTeamId != teamId2) {
        ...
        if (oneOrBothTeamsWasReady) {
            if (readyTeam1) {
                this->m_tree_ready_teams->m_root = this->m_tree_ready_teams->deleteNode(
                        this->m_tree_ready_teams->m_root, teamId1, 0, 0);
                this->m_num_good_teams--;
            }
            if (readyTeam2) {
                this->m_tree_ready_teams->m_root = this->m_tree_ready_teams->deleteNode(
                        this->m_tree_ready_teams->m_root, teamId2, 0, 0);
                this->m_num_good_teams--;
            }
            ready_team newReadyTeam(newTeamId, newTeam);
            this->m_tree_ready_teams->m_root = this->m_tree_ready_teams->insertNode(this->m_tree_ready_teams->m_root,
                                                                                    newTeamId, 0, 0, newReadyTeam);
            this->m_num_good_teams++;
        }
    ...
    for (int i = 0; i < numPlayersTeam1; i++) {
        ...
        arrayPlayersID1[i]->m_element.m_player->m_element.m_my_team = newTeam;
    ...
    for (int i = 0; i < numPlayersTeam2; i++) {
        ...
        arrayPlayersID2[i]->m_element.m_player->m_element.m_my_team = newTeam;
    }

    newTeam->m_element.m_games_played_by_team =
            team1->m_element.m_games_played_by_team + team2->m_element.m_games_played_by_team;
    ...
    newTeam->m_element.m_tree_players_in_team_by_id->merge(arrayPlayersID1, arrayPlayersID2, mergeArrayID,
                                                           numPlayersTeam1, numPlayersTeam2);
    newTeam->m_element.m_tree_players_in_team_by_score->merge(arrayPlayersScore1, arrayPlayersScore2, mergeArrayScore,
                                                              numPlayersTeam1, numPlayersTeam2);

    try {
        newTeam->m_element.m_tree_players_in_team_by_id->m_root = newTeam->m_element.m_tree_players_in_team_by_id->arrayToTree(
                mergeArrayID, 0, (numPlayersTeam1) + (numPlayersTeam2) - 1);
        newTeam->m_element.m_tree_players_in_team_by_score->m_root = newTeam->m_element.m_tree_players_in_team_by_score->arrayToTree(
                mergeArrayScore, 0, (numPlayersTeam1) + (numPlayersTeam2) - 1);
    }
    catch (std::bad_alloc &) {
        return StatusType::ALLOCATION_ERROR;
    }
    ...
    newTeam->m_element.m_tree_players_in_team_by_id->m_highest_key1 = mergeArrayID[(numPlayersTeam1) +
                                                                                   (numPlayersTeam2) - 1]->m_key1;
    newTeam->m_element.m_tree_players_in_team_by_id->m_highest_key2 = mergeArrayID[(numPlayersTeam1) +
                                                                                   (numPlayersTeam2) - 1]->m_key2;
    newTeam->m_element.m_tree_players_in_team_by_id->m_highest_key3 = mergeArrayID[(numPlayersTeam1) +
                                                                                   (numPlayersTeam2) - 1]->m_key3;

    newTeam->m_element.m_tree_players_in_team_by_score->m_highest_key1 = mergeArrayScore[(numPlayersTeam1) +
                                                                                         (numPlayersTeam2) - 1]->m_key1;
    newTeam->m_element.m_tree_players_in_team_by_score->m_highest_key2 = mergeArrayScore[(numPlayersTeam1) +
                                                                                         (numPlayersTeam2) - 1]->m_key2;
    newTeam->m_element.m_tree_players_in_team_by_score->m_highest_key3 = mergeArrayScore[(numPlayersTeam1) +
                                                                                         (numPlayersTeam2) - 1]->m_key3;
    ...
    if (newTeamId != teamId1 && newTeamId != teamId2) {
        ...
        team1->m_element.m_tree_players_in_team_by_score->m_root = nullptr;
        team1->m_element.m_tree_players_in_team_by_id->m_root = nullptr;
        team2->m_element.m_tree_players_in_team_by_score->m_root = nullptr;
        team2->m_element.m_tree_players_in_team_by_id->m_root = nullptr;
        ...
        team2->m_element.m_tree_players_in_team_by_score->m_root = nullptr;
        team2->m_element.m_tree_players_in_team_by_id->m_root = nullptr;
        Node<Team> *nextTeam = this->m_tree_teams_by_id->closestHigherNode(this->m_tree_teams_by_id->m_root, team2);
        int nextTeamId = 0;
        if (nextTeam) {
            nextTeamId = nextTeam->m_key1;
        }
            this->remove_team(team2->m_element.m_team_id);
            nextTeam = this->m_tree_teams_by_id->findNode(this->m_tree_teams_by_id->m_root, nextTeamId, 0, 0);
            if (nextTeam){
                Node<player_in_team> **arrayPlayersID = new Node<player_in_team> *[nextTeam->m_element.m_num_players];
                Node<player_in_team> **arrayPlayersScore = new Node<player_in_team> *[nextTeam->m_element.m_num_players];
                nextTeam->m_element.m_tree_players_in_team_by_id->inOrderVisitUnite(
                        nextTeam->m_element.m_tree_players_in_team_by_id->m_root, arrayPlayersID, 0);
                nextTeam->m_element.m_tree_players_in_team_by_score->inOrderVisitUnite(
                        nextTeam->m_element.m_tree_players_in_team_by_score->m_root, arrayPlayersScore, 0);
                for (int i = 0; i <nextTeam->m_element.m_num_players; i++){
                    arrayPlayersID[i]->m_element.m_player->m_element.m_my_team = nextTeam;
                    arrayPlayersScore[i]->m_element.m_player->m_element.m_my_team = nextTeam;
                }
                delete[] arrayPlayersID;
                delete[] arrayPlayersScore;
        }
        } else if (newTeamId == teamId2) {
            team1->m_element.m_num_players = 0;
            team1->m_element.m_tree_players_in_team_by_score->m_root = nullptr;
            team1->m_element.m_tree_players_in_team_by_id->m_root = nullptr;
        Node<Team> *nextTeam = this->m_tree_teams_by_id->closestHigherNode(this->m_tree_teams_by_id->m_root, team1);
        int nextTeamId = 0;
        if (nextTeam) {
            nextTeamId = nextTeam->m_key1;
        }
    ...
    StatusType world_cup_t::get_all_players(int teamId, int *const output) {
        if (teamId == 0) {
            ...
            // saving all of the players in the array output
            currentTeam->m_element.m_tree_players_in_team_by_score->inOrderVisit(
                    currentTeam->m_element.m_tree_players_in_team_by_score->m_root, output, 0);
        }
        return StatusType::SUCCESS;
    }
    ...
    output_t<int> world_cup_t::get_closest_player(int playerId, int teamId) {
        ...
        Node<player_in_team> *playerInTeamById = currentTeam->m_element.m_tree_players_in_team_by_id->findNode(
                currentTeam->m_element.m_tree_players_in_team_by_id->m_root, playerId, 0, 0);
        if (!playerInTeamById) {
            return StatusType::FAILURE;
        }
        Node<player_in_scoreboard> *playerInScoreBoard = playerInTeamById->m_element.m_player->m_element.m_player_in_scoreboard;
        Node<player_in_scoreboard> *closest = this->m_tree_players_by_score->closerBetweenTwoOptions(playerInScoreBoard,
                                                                                                     playerInScoreBoard->m_element.m_next_player_by_score,
                                                                                                     playerInScoreBoard->m_element.m_prev_player_by_score);
    ...
    output_t<int> world_cup_t::knockout_winner(int minTeamId, int maxTeamId) {
        ...
        int numGoodTeams = this->m_tree_ready_teams->inOrderVisitBetweenRange(currTeam, goodTeams, 0, minTeamId,
                                                                              maxTeamId);
        if (numGoodTeams == 0){
            return StatusType::FAILURE;
        }
